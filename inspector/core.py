...
class InspectorEngine:
    ...
    def scan(self, target_path: str = None) -> InspectionSummary:
        """Ejecuta una inspección completa sobre la ruta objetivo."""
        target = target_path or self.config.target_path
        start_time = time.time()

        files_to_scan = self.discover_files(target)
        ...
        for file_path in files_to_scan:
            ...
            # Ejecutar todos los escáneres sobre el archivo
            rel_file_path = os.path.relpath(file_path, start=target)
